...
public class MultipleBoolSettingComponent extends ExpandableComponent implements PropertyComponent {
    ...
    public void drawComponent(ScaledResolution scaledResolution, int mouseX, int mouseY) {
        ...
        for (BooleanSetting booleanSetting : listSetting.getBoolSettings()) {
            ...
            mc.rubik_16.drawCenteredStringWithOutline(getName(), x + width / 2 + fun.rich.client.ui.clickgui.Panel.X_ITEM_OFFSET, dropDownBoxY + 3F, new Color(222, 222, 222).getRGB());
            if (isExpanded()) {
    ...
    private void handleRender(int x, int y, int width, int textColor) {
        ...
        for (BooleanSetting e : listSetting.getBoolSettings()) {
            if (e.getBoolValue()) {
                mc.rubik_15.drawCenteredBlurredString(e.getName(), x + fun.rich.client.ui.clickgui.Panel.X_ITEM_OFFSET + width / 2 + 0.5f, y + 2.5F, 8, RenderUtils.injectAlpha(new Color(ClickGUI.color.getColorValue()), 60),ClickGUI.color.getColorValue());
            } else {
                mc.rubik_15.drawCenteredString(e.getName(), x + fun.rich.client.ui.clickgui.Panel.X_ITEM_OFFSET + width / 2 + 0.5f, y + 2.5F, Color.GRAY.getRGB());
