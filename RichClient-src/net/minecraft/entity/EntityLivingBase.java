...
import fun.rich.client.feature.impl.visual.JumpCircle;
...
public abstract class EntityLivingBase extends Entity {
    ...
    private int getArmSwingAnimationEnd() {
        if (Rich.instance.featureManager.getFeature(SwingAnimations.class).isEnabled()) {
     ...
     */
    public long getDesyncTick() {
        if (getActiveItemStack().getItem() instanceof ItemShield) {
            if (!isStarted) {
                desyncTicks = System.currentTimeMillis();
            }
            isStarted = true;
        } else {
            isStarted = false;
        }
        return System.currentTimeMillis() - desyncTicks;
    }

    public long ticksBlock = 0;

    public void onUpdate() {
        super.onUpdate();
        this.updateActiveHand();
        ticksBlock = getDesyncTick();
        if (!this.world.isRemote) {
    ...
    public boolean canBePushed() {
        if (Rich.instance.featureManager.getFeature(NoPush.class).isEnabled() && NoPush.players.getBoolValue() && this instanceof EntityPlayerSP) {
            return false;
        }
    ...
    public boolean isActiveItemStackBlocking(int ticks) {
        if (this.isHandActive() && !this.activeItemStack.func_190926_b()) {
            ...
            if (item.getItemUseAction(this.activeItemStack) != EnumAction.BLOCK) {
                ...
                return item.getMaxItemUseDuration(this.activeItemStack) - this.activeItemStackUseCount >= ticks;
            }
        } else {
            return false;
        }
