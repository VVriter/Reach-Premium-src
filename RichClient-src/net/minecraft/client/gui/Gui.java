...
import fun.rich.client.utils.render.GLUtil;
...
public class Gui
{
    ...
    public static void drawGradientRect(float f8, float sY, double d, double e, int startColor, int endColor)
    {
        ...
        bufferbuilder.pos((double)d, (double)sY, (double)zLevel).color(f1, f2, f3, f).endVertex();
        bufferbuilder.pos((double)f8, (double)sY, (double)zLevel).color(f5, f6, f7, f4).endVertex();
        bufferbuilder.pos((double)f8, (double)e, (double)zLevel).color(f5, f6, f7, f4).endVertex();
