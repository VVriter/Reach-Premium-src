...
import fun.rich.client.feature.impl.visual.ScoreboardFeatures;
...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (scoreobjective1 != null) {
            ...
            EventManager.call(e);
            if (!(Rich.instance.featureManager.getFeature(ScoreboardFeatures.class).isEnabled() && ScoreboardFeatures.noScore.getBoolValue())) {
        ...
        if ((this.mc.gameSettings.keyBindPlayerList.isKeyDown() || this.overlayPlayerList.addition > 0.0F) && (!this.mc.isIntegratedServerRunning() || this.mc.player.connection.getPlayerInfoMap().size() > 1 || scoreobjective1 != null)) {
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score score1 : collection) {
            ...
            if (Rich.instance.featureManager.getFeature(NameProtect.class).isEnabled() && NameProtect.scoreboardSpoof.getBoolValue()) {
                this.getFontRenderer().drawString(s1.replace(Minecraft.getMinecraft().player.getName().substring(0, 2), TextFormatting.WHITE + "Rich Premium                                                                                                 "), l1, k, 553648127);
            } else {
