...
import fun.rich.client.command.impl.FakeNameCommand;
import fun.rich.client.event.EventManager;
import fun.rich.client.event.events.impl.player.EventUpdateLiving;
import fun.rich.client.feature.impl.combat.KeepSprint;
import fun.rich.client.feature.impl.misc.Disabler;
import fun.rich.client.feature.impl.misc.NameProtect;
import fun.rich.client.feature.impl.movement.ElytraStrafe;
import fun.rich.client.feature.impl.movement.Flight;
import fun.rich.client.feature.impl.player.FreeCam;
import fun.rich.client.feature.impl.player.NoClip;
import fun.rich.client.feature.impl.player.NoPush;
...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    public void onUpdate() {
        this.noClip = Rich.instance.featureManager.getFeature(NoClip.class).isEnabled() || Rich.instance.featureManager.getFeature(FreeCam.class).isEnabled() || this.isSpectator();
    ...
    public float getDigSpeed(IBlockState state, ItemStack itemstack) {
        ...
        if (this.isPotionActive(MobEffects.HASTE)) {
            f = 1.0F + (float) (this.getActivePotionEffect(MobEffects.HASTE).getAmplifier() + 1);
        }
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag5) {
                        if (i > 0) {
                            ...
                            if (Rich.instance.featureManager.getFeature(KeepSprint.class).isEnabled()) {
    ...
    public String getName() {
        ...
        if (this == (Minecraft.getMinecraft()).player && FakeNameCommand.canChange) {
            ...
            str = (Rich.instance.featureManager.getFeature(NameProtect.class).isEnabled() && NameProtect.myName.getBoolValue() && this == (Minecraft.getMinecraft()).player) ? "Protected" : this.gameProfile.getName();
        }
        return str;
    }
    ...
    public boolean isPushedByWater() {
        if (Rich.instance.featureManager.getFeature(NoPush.class).isEnabled() && NoPush.water.getBoolValue()) {
            return false;
        }
        return !this.capabilities.isFlying;
    }
    ...
    public static double animationY;
    ...
    public float getEyeHeight() {
        ...
        if (this.isPlayerSleeping()) {
            ...
            if (this.isElytraFlying() || this.height == 0.6F) {
                if (Rich.instance.featureManager.getFeature(Flight.class).isEnabled() && Flight.flyMode.currentMode.equals("Matrix Elytra") || Rich.instance.featureManager.getFeature(ElytraStrafe.class).isEnabled() || Rich.instance.featureManager.getFeature(Disabler.class).isEnabled()) {
                    f = 1.62F;
                } else {
                    f = 0.4F;
                }
    ...
    public ItemStack getEquipmentInSlot(int p_71124_1_) {
        return p_71124_1_ == 0 ? this.inventory.getCurrentItem() : this.inventory.armorInventory.get(p_71124_1_ - 1);
    }
