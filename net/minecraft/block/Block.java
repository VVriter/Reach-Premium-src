...
import fun.rich.client.feature.impl.player.AutoTool;
...
public class Block
{
    ...
    public boolean canCollideCheck(IBlockState state, boolean hitIfLiquid) {
        for (Block block : Objects.requireNonNull(NoInteract.getRightClickableBlocks())) {
            if (state.getBlock() != block || !Rich.instance.featureManager.getFeature(NoInteract.class).isEnabled()) continue;
            return false;
        }
    ...
    public float getPlayerRelativeBlockHardness(IBlockState state, EntityPlayer player, World worldIn, BlockPos pos)
    {
        ...
        {
            AutoTool autoTool = ((AutoTool) Rich.instance.featureManager.getFeature(AutoTool.class));
            return !player.canHarvestBlock(state) ? player.getDigSpeed(state, (autoTool.isEnabled() && autoTool.silentSwitch.getBoolValue()) ? player.inventory.getStackInSlot(autoTool.itemIndex) : player.getHeldItemMainhand()) / f / 100.0F : player.getDigSpeed(state,(autoTool.isEnabled() && autoTool.silentSwitch.getBoolValue()) ? player.inventory.getStackInSlot(autoTool.itemIndex) : player.getHeldItemMainhand()) / f / 30.0F;        }
    }
