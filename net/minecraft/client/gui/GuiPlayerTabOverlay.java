...
public class GuiPlayerTabOverlay extends Gui {
    public static Ordering<NetworkPlayerInfo> ENTRY_ORDERING = Ordering.from(new GuiPlayerTabOverlay.PlayerComparator());
    private static Minecraft mc = Minecraft.getMinecraft();
    private final GuiIngame guiIngame;
    private ITextComponent footer;
    private ITextComponent header;
    public float posAnimationY, addition = 0;
    ...
    public void renderPlayerlist(int width, Scoreboard scoreboardIn, @Nullable ScoreObjective scoreObjectiveIn) {
        ...
        if (this.mc.gameSettings.keyBindPlayerList.isKeyDown()) {
            if (addition < 1.0F) addition += 1.5F * Rich.deltaTime();
        } else {
            if (addition > 0.0F) addition -= 1.5F * Rich.deltaTime();
        }

        addition = MathHelper.clamp(0F, 1F, addition);

        posAnimationY = MathHelper.EaseOutBack(-sr.getScaledHeight(), k1, addition);

        GL11.glPushMatrix();
        GlStateManager.translate(width / 2.0F, posAnimationY / 2.0F, 1F);
        GlStateManager.translate(-width / 2.0F, posAnimationY / 2.0F, 1F);
        if (this.header != null) {
        ...
        for (int k4 = 0; k4 < l3; ++k4) {
            ...
            if (k4 < list.size()) {
                ...
                if (Rich.instance.featureManager.getFeature(NameProtect.class).isEnabled()) {
                    if (NameProtect.tabSpoof.getBoolValue() && !s4.contains(mc.player.getName())) {
                        s4 = "\247aProtected";
                    }
                    if (NameProtect.myName.getBoolValue()) {
                        s4 = s4.replace(this.mc.player.getName(), TextFormatting.GREEN + "Protected");
                    }
                }
