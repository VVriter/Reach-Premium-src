...
import fun.rich.client.feature.impl.hud.NoOverlay;
import fun.rich.client.feature.impl.player.NoInteract;
import fun.rich.client.feature.impl.visual.FogColor;
import fun.rich.client.feature.impl.visual.WorldFeatures;
...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            for (int j = 0; j < list.size(); ++j) {
                ...
                if (entity1 instanceof EntityArmorStand && Rich.instance.featureManager.getFeature(NoInteract.class).isEnabled() && NoInteract.armorStands.getBoolValue()) {
                    return;
                }
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        ...
        double zoomSpeedPress = 0.05D;
        double zoomSpeedUNPress = 0.03D;
        if (flag) {
            if (!Config.zoomMode) {
            ...
            if (this.zoomSpeed < 5.0F)
                this.zoomSpeed = (float) (this.zoomSpeed + zoomSpeedPress * Minecraft.frameTime * 0.1D);
        } else {
            if (this.zoomSpeed > 1.0F)
                this.zoomSpeed = (float) (this.zoomSpeed - zoomSpeedUNPress * Minecraft.frameTime * 0.1D);
        ...
        IBlockState iblockstate = ActiveRenderInfo.getBlockStateAtEntityViewpoint((World) this.mc.world, entity, partialTicks);
    ...
    private void hurtCameraEffect(float partialTicks) {
        if (Rich.instance.featureManager.getFeature(NoOverlay.class).isEnabled() && NoOverlay.noHurtCam.getBoolValue()) {
            return;
        }
    ...
    public void orientCamera(float partialTicks) {
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            double d3 = (double) (this.thirdPersonDistancePrev + (2.0F - 2.5));
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    if (raytraceresult != null) {
                        ...
                        if (Rich.instance.featureManager.getFeature(NoOverlay.class).isEnabled() && NoOverlay.cameraClip.getBoolValue()) {
                ...
                GlStateManager.translate(0F, 0.0F, (float) (-d3));
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
            if (!this.mc.gameSettings.debugCamEnable) {
                ...
                if (entity instanceof EntityAnimal) {
                    ...
                    f6 = entityanimal1.prevRotationYawHead
                            + (entityanimal1.rotationYawHead - entityanimal1.prevRotationYawHead) * partialTicks
                            + 180.0F;
                }

                IBlockState iblockstate1 = ActiveRenderInfo.getBlockStateAtEntityViewpoint(this.mc.world, entity,
                        partialTicks);
                Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_CameraSetup_Constructor, this,
                        entity, iblockstate1, partialTicks, f6, f7, f8);
    ...
    private void updateLightmap(float partialTicks) {
        if (this.lightmapUpdateNeeded) {
            ...
            if (world != null) {
                ...
                if (Rich.instance.featureManager.getFeature(WorldFeatures.class).isEnabled() && WorldFeatures.worldColor.getBoolValue()) {
    ...
    protected void renderRainSnow(float partialTicks) {
        ...
        if (Rich.instance.featureManager.getFeature(WorldFeatures.class).isEnabled() && WorldFeatures.snow.getBoolValue()) {
            ...
            if (f > 0.0F) {
                ...
                for (int k1 = k - i1; k1 <= k + i1; k1++) {
                    for (int l1 = i - i1; l1 <= i + i1; l1++) {
                        ...
                        boolean check = Rich.instance.featureManager.getFeature(WorldFeatures.class).isEnabled() && WorldFeatures.snow.getBoolValue() ? true : ((biome.canRain() || biome.getEnableSnow()));
                        if (check) {
                            ...
                            if (k2 != l2) {
                                ...
                                float f2 = biome.getFloatTemperature((BlockPos) blockpos$mutableblockpos);
                                if (j1 != 1) {
                                    if (j1 >= 0)
                                        tessellator.draw();
                                    j1 = 1;
                                ...
                                int j4 = i4 >> 16 & 0xFFFF;
                                int k4 = i4 & 0xFFFF;
                                bufferbuilder.pos(l1 - d3 + 0.5D, l2, k1 - d4 + 0.5D).tex(0.0D + d9, k2 * 0.25D + d8 + d10).color(red, green, blue, f5).lightmap(j4, k4).endVertex();
                                bufferbuilder.pos(l1 + d3 + 0.5D, l2, k1 + d4 + 0.5D).tex(1.0D + d9, k2 * 0.25D + d8 + d10).color(red, green, blue, f5).lightmap(j4, k4).endVertex();
                                bufferbuilder.pos(l1 + d3 + 0.5D, k2, k1 + d4 + 0.5D).tex(1.0D + d9, l2 * 0.25D + d8 + d10).color(red, green, blue, f5).lightmap(j4, k4).endVertex();
                                bufferbuilder.pos(l1 - d3 + 0.5D, k2, k1 - d4 + 0.5D).tex(0.0D + d9, l2 * 0.25D + d8 + d10).color(red, green, blue, f5).lightmap(j4, k4).endVertex();
                            }
                        }
                    }
                }
                    ...
                    for (int l1 = i - i1; l1 <= i + i1; l1++) {
                        ...
                        if (biome.canRain() || biome.getEnableSnow()) {
                            ...
                            if (k2 != l2) {
                                this.random.setSeed((l1 * l1 * 3121 + l1 * 45238971 ^ k1 * k1 * 418711 + k1 * 13761));
                                blockpos$mutableblockpos.setPos(l1, k2, k1);
                                float f1 = biome.getFloatTemperature((BlockPos) blockpos$mutableblockpos);
                                if (worldClient.getBiomeProvider().getTemperatureAtHeight(f1, j2) >= 0.15F) {
                                    if (j1 != 0) {
                                        if (j1 >= 0)
                                            tessellator.draw();
                                        j1 = 0;
                                    ...
                                    int k3 = j3 >> 16 & 0xFFFF;
                                    int l3 = j3 & 0xFFFF;
                                    bufferbuilder.pos(l1 - d3 + 0.5D, l2, k1 - d4 + 0.5D).tex(0.0D, k2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
                                    bufferbuilder.pos(l1 + d3 + 0.5D, l2, k1 + d4 + 0.5D).tex(1.0D, k2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
                                    bufferbuilder.pos(l1 + d3 + 0.5D, k2, k1 + d4 + 0.5D).tex(1.0D, l2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
                                    bufferbuilder.pos(l1 - d3 + 0.5D, k2, k1 - d4 + 0.5D).tex(0.0D, l2 * 0.25D + d5).color(1.0F, 1.0F, 1.0F, f3).lightmap(k3, l3).endVertex();
                                } else {
                                    if (j1 != 1) {
                                        if (j1 >= 0)
                                            tessellator.draw();
                                    ...
                                    int j4 = i4 >> 16 & 0xFFFF;
                                    int k4 = i4 & 0xFFFF;
                                    bufferbuilder.pos(l1 - d3 + 0.5D, l2, k1 - d4 + 0.5D).tex(0.0D + d9, k2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
                                    bufferbuilder.pos(l1 + d3 + 0.5D, l2, k1 + d4 + 0.5D).tex(1.0D + d9, k2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
                                    bufferbuilder.pos(l1 + d3 + 0.5D, k2, k1 + d4 + 0.5D).tex(1.0D + d9, l2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
                                    bufferbuilder.pos(l1 - d3 + 0.5D, k2, k1 - d4 + 0.5D).tex(0.0D + d9, l2 * 0.25D + d8 + d10).color(1.0F, 1.0F, 1.0F, f4).lightmap(j4, k4).endVertex();
                                }
                            }
                        }
                    }
    ...
    private void setupFog(int startCoords, float partialTicks) {
        ...
        if (f >= 0.0F) {
            ...
            if (startCoords == -1) {
                ...
                float dist = Rich.instance.featureManager.getFeature(FogColor.class).isEnabled() ? -FogColor.distance.getNumberValue() : Config.getFogStart();
    ...
    public void func_190565_a(ItemStack p_190565_1_) {
        if (Rich.instance.featureManager.getFeature(NoOverlay.class).isEnabled() && NoOverlay.antiTotem.getBoolValue()) {
            return;
        }
