...
public abstract class RenderLivingBase<T extends EntityLivingBase> extends Render<T>
{
    ...
    protected void renderModel(T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor) {
        ...
        boolean chamsCheck = Rich.instance.featureManager.getFeature(Chams.class).isEnabled() && entitylivingbaseIn instanceof EntityPlayer;
        ...
        if (flag || flag1) {
            ...
            if (chamsCheck && mode.equalsIgnoreCase("Fill")) {
                ...
                if (!Chams.clientColor.getBoolValue()) {
                    GlStateManager.color((float)color.darker().getRed() / 255.0f, (float)color.darker().darker().getGreen() / 255.0f, (float)color.darker().getBlue() / 255.0f, Chams.chamsAlpha.getNumberValue());
                } else {
                    GlStateManager.color((float)ClientHelper.getClientColor().darker().getRed() / 255.0f, (float)ClientHelper.getClientColor().darker().getGreen() / 255.0f, (float)ClientHelper.getClientColor().darker().getBlue() / 255.0f, Chams.chamsAlpha.getNumberValue());
                }
                GL11.glDisable(2929);
            ...
            if (chamsCheck && mode.equalsIgnoreCase("Fill")) {
                ...
                if (!Chams.clientColor.getBoolValue()) {
                    GlStateManager.color((float)color.getRed() / 255.0f, (float)color.getGreen() / 255.0f, (float)color.getBlue() / 255.0f, Chams.chamsAlpha.getNumberValue());
                } else {
                    GlStateManager.color((float)ClientHelper.getClientColor().getRed() / 255.0f, (float)ClientHelper.getClientColor().getGreen() / 255.0f, (float)ClientHelper.getClientColor().getBlue() / 255.0f, Chams.chamsAlpha.getNumberValue());
                }
                this.mainModel.render(entitylivingbaseIn, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scaleFactor);
                GL11.glEnable(3553);
    ...
    public void renderName(T entity, double x, double y, double z) {
        if (!Reflector.RenderLivingEvent_Specials_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Specials_Pre_Constructor, entity, this, x, y, z)) {
            if (this.canRenderName(entity)) {
                ...
                if (d0 < (double) (f * f)) {
                    EventRenderPlayerName eventNameTag = new EventRenderPlayerName(entity, entity.getDisplayName().getFormattedText());
