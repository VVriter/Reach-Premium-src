...
import fun.rich.client.feature.impl.misc.ChatHistory;
...
import fun.rich.protection.AES;
...
public class Minecraft implements IThreadListener, ISnooperInfo {
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        Rich.instance.init();
        Rich.instance.configManager.loadConfig("default");
        SplashProgress.setProgress(7);

        Rich.instance.client.connect("51.38.166.157", 9898);

        DataInputStream in = new DataInputStream(Rich.instance.client.getSocket().getInputStream());
        DataOutputStream out = new DataOutputStream(Rich.instance.client.getSocket().getOutputStream());

        out.writeUTF(AES.encrypt("H:" + Protection.jg4j1));
        RichMainMenu menu = new RichMainMenu();
        out.writeUTF(AES.encrypt("Z:" + menu.getClass().getName()));


        this.displayGuiScreen(Rich.instance.client.getScreen(AES.decrypt(in.readUTF(), "912389123")));

        out.writeUTF(AES.encrypt("FLEX:0"));

        Client.flex = Integer.parseInt(AES.decrypt(in.readUTF(), "65461238"));

        if (Client.flex != 6581293) {
            shutdown();
        }

        out.writeUTF(AES.encrypt("data"));

        UserData.instance().setName(AES.decrypt(in.readUTF(), "712723"));
        UserData.instance().setID(AES.decrypt(in.readUTF(), "912378"));
        UserData.instance().setLicenseDate(AES.decrypt(in.readUTF(), "965124"));
        UserData.instance().setUID(AES.decrypt(in.readUTF(), "54123122"));
        try {
        ...
        if (this.serverName != null) {
            ...
            this.displayGuiScreen(new RichMainMenu());
        }
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle(Rich.instance.name + " " + Rich.instance.type + " - https://vk.com/richsense");
    ...
    public void displayGuiScreen(@Nullable GuiScreen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.world == null) {
            guiScreenIn = new RichMainMenu();
        } else if (guiScreenIn == null && this.player.getHealth() <= 0.0F) {
            guiScreenIn = new GuiGameOver((ITextComponent) null);
        }

        if (guiScreenIn instanceof RichMainMenu || guiScreenIn instanceof GuiMultiplayer) {
            this.gameSettings.showDebugInfo = false;
            this.ingameGUI.getChatGUI().clearChatMessages(!Rich.instance.featureManager.getFeature(ChatHistory.class).isEnabled());
        }
